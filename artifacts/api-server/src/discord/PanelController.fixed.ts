...
export class PanelController {
  ...
  // ==================== SCORE CONFIRMATION - VERSION CORRIGEE ====================
  ...
  private async confirmProposedScore(interaction: ButtonInteraction, matchId: string, contextId?: string): Promise<void> {
    ...
    // FIX: Déterminer la vraie équipe qui a proposé
    let proposingTeamId = (match as any).proposedByTeamId as string | undefined;
    if (!proposingTeamId) {
      if (teamA.memberIds.includes(match.proposedBy ?? "")) proposingTeamId = teamA.id;
      else if (teamB.memberIds.includes(match.proposedBy ?? "")) proposingTeamId = teamB.id;
      else proposingTeamId = teamA.memberIds.includes(uid) ? teamB.id : teamA.id; // fallback si proposé avant le fix
    }

    const proposingTeam = proposingTeamId === teamA.id ? teamA : teamB;
    const opponentTeam = proposingTeamId === teamA.id ? teamB : teamA;

    // FIX: Déterminer l'équipe de celui qui clique
    const userTeam = Object.values(state.teams).find(t => t.memberIds.includes(uid));

    if (!staff) {
      // Règle 1: Le capitaine de l'équipe qui a entré le score est bloqué
      if (userTeam?.id === proposingTeamId || uid === match.proposedBy) {
        return safeReply(interaction, { content: `⛔ Tu ne peux pas confirmer ton propre score. C'est à l'équipe adverse **${opponentTeam.tag}** de confirmer.`, ephemeral: true });
      }
      // Règle 2: Il faut être de l'équipe adverse
      if (!userTeam || userTeam.id !== opponentTeam.id) {
        return safeReply(interaction, { content: `⛔ Seul le capitaine adverse (<@${opponentTeam.captainId}>) peut confirmer.`, ephemeral: true });
      }
      // Règle 3: Il faut être CAPITAINE adverse
      if (uid !== opponentTeam.captainId) {
        return safeReply(interaction, { content: `⛔ Seul le capitaine adverse peut confirmer. C'est <@${opponentTeam.captainId}>, pas toi.`, ephemeral: true });
      }
    }
    ...
    await safeReply(interaction, { 
      content: `✅ Score confirmé par <@${uid}>! Vainqueur: **${state.teams[result.winnerId!]?.name ?? result.winnerId}**\nMatch ${matchId} terminé.`, 
    });
  ...
  private async replyMyMatch(interaction: ButtonInteraction | ChatInputCommandInteraction): Promise<void> {
    ...
    const myMatches = Object.values(state.matches).filter(m => m.teamAId === myTeam.id || m.teamBId === myTeam.id);
  ...
  private async showScoreMatchPicker(interaction: ButtonInteraction): Promise<void> {
    ...
    const myTeam = Object.values(state.teams).find(t => t.memberIds.includes(uid));
    if (!myTeam) return safeReply(interaction, { content: "Tu n'es dans aucune team.", ephemeral: true });
    const available = Object.values(state.matches).filter(m => (m.teamAId === myTeam.id || m.teamBId === myTeam.id) && m.status === "in-progress");
  ...
  // FIX LITIGE - meme logique que confirmation
  private async openDispute(interaction: ButtonInteraction, matchId: string): Promise<void> {
    ...
    const teamA = match.teamAId ? state.teams[match.teamAId] : undefined;
    const teamB = match.teamBId ? state.teams[match.teamBId] : undefined;
    if (!teamA || !teamB) return safeReply(interaction, { content: "Teams introuvables.", ephemeral: true });
    ...
    if (!proposingTeamId) proposingTeamId = match.teamAId;

    const opponentTeam = proposingTeamId === teamA.id ? teamB : teamA;
    const userTeam = Object.values(state.teams).find(t => t.memberIds.includes(uid));

    if (!staff) {
      if (userTeam?.id === proposingTeamId) {
        return safeReply(interaction, { content: `⛔ Tu ne peux pas ouvrir un litige sur ton propre score.`, ephemeral: true });
      }
      if (!userTeam || userTeam.id !== opponentTeam.id) {
        return safeReply(interaction, { content: `⛔ Seul le capitaine adverse peut ouvrir ce litige.`, ephemeral: true });
      }
      if (uid !== opponentTeam.captainId) {
        return safeReply(interaction, { content: `⛔ Seul le capitaine adverse (<@${opponentTeam.captainId}>) peut ouvrir ce litige.`, ephemeral: true });
      }
    }

    await this.store.mutateGuild(guild.id, (s) => {
      const m = s.matches[matchId];
      if (m) m.status = "disputed";
    });

    await safeReply(interaction, {
      content: `⚠️ **Litige ouvert sur le Match ${matchId} par <@${uid}>!**`,
      components: [
        new ActionRowBuilder<ButtonBuilder>().addComponents(
